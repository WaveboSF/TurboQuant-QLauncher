...
class TurboQuantQLauncher(tk.Tk):
    ...
    def _show_paths_dialog(self, on_close=None):
        ...
        def _save():
            ...
            # Validate active server path — must be a real llama-server binary,
            # not the launcher shell script or some unrelated file. If invalid,
            # silently fall back to the first valid slot so the user isn't stuck.
            active = server_var.get().strip()
